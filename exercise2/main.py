...
class InvalidStepSizeError(Exception):
    pass
...
class BasePlotter:
    ...
    def _plot_setup(self):
        """Runs basic common setup for plotters."""
...
class FunctionPlotter(BasePlotter):
    ...
    """
    def __init__(self, settings: PlotSettings, func: Callable[[float], float], start_x: float, stop_x: float, step: float = 1e-3):
        if start_x > stop_x:
            raise InvalidRangeError("start_x cannot be bigger than stop_x")
        if step < 0 or step > stop_x - start_x:
            raise InvalidStepSizeError("step is either negative or bigger than the interval")
...
class PeriodicFunctionPlotter(FunctionPlotter):
    """Plotter that plots a number of periods of a periodic function. The periodicity of the function is detected in the interval `start_x` to `stop_x`.
    ...
    def __init__(self, settings: PlotSettings, periodic_func: Callable[[float], float], start_x: float, stop_x: float,  periods: int = 2, step: float = 1e-3):
        if start_x > stop_x:
            raise InvalidRangeError("start_x cannot be bigger than stop_x")
        if step < 0 or step > stop_x - start_x:
            raise InvalidStepSizeError("step is either negative or bigger than the interval")
        ...
        # Make arrays in order to detect periodicity. This has potential for optimization since this has to be done again when plotting
    ...
    def plot(self):
        """Generates the plot"""
        plt.title(f"Showing {self._periods} periods")
        super().plot()
...
def main():
    ...
    plotter = PeriodicFunctionPlotter(PlotSettings("Plot of h(x)"),h ,0, 1000)
    plotter.plot()
